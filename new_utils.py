...
############################################################
def calc_rpn1(C, img_data, width, height, resized_width, resized_height, img_length_calc_function,mode):
    ...
    if len(pos_locs[0]) > C.rpn_num_train_examples/2:
        val_locs = random.sample(range(len(pos_locs[0])), len(pos_locs[0]) - C.rpn_num_train_examples/2)
        y_is_box_valid[0, pos_locs[0][val_locs], pos_locs[1][val_locs], pos_locs[2][val_locs]] = 0
        num_pos = C.rpn_num_train_examples/2

    if len(neg_locs[0]) + num_pos > C.rpn_num_train_examples:
...
def get_training_batch_classifier(C, X2_train, Y1_train, Y2_train, IouS_train, mode, overlap_others):
    ...
    # Dell examples wich overlap with a gt others if mode is 'P'
    if (mode == 'P'):
